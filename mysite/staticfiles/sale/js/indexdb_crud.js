...
async function saveBillOffline(payload) {
  try {
    ...
    console.log(" Bill added to pendingBills:", result);
    return result;
  } catch (err) {
...
async function syncoffline_bills() {
  ...
  try {
    ...
    console.error("[Sale Sync] Failed to read local DB:", dbErr);
    ...
    await Promise.all(
      batchGroup.map(async (chunk) => {
        ...
        try {
          ...
          console.error("[Sale Sync] Network/HTTP error for batch:", networkErr);
        ...
        if (data && data.success) {
          ...
          try {
            ...
            // Unlikely but log it — records were saved but local delete failed
            console.error("[Sale Sync] Failed to delete local records after successful sync:", deleteErr);
          ...
          console.error("[Sale Sync] Server rejected batch:", serverMsg, "| Saved bills:", data?.saved_count, "| Failed:", data?.failed_count);
          totalFailed += chunk.length;
          chunk.forEach(b => failedBillDetails.push({
            local_id: b.local_id,
            reason: serverMsg
  ...
  if (totalFailed === 0 && totalSynced > 0) {
    ...
    console.warn("[Sale Sync] Failures:", failedBillDetails);
...
async function searchProductOffline(term) {
  ...
  try {
    ...
    if (!cachedInventory || (now - cachedInventoryTime > 60000)) {
      ...
      console.log(`[searchProductOffline] Loading 130k items from IndexedDB into memory took: ${(tDb1 - tDb0).toFixed(2)} ms`);
    ...
    const result = products.filter((p) => {
      return (
        ...
        (p.alias_name && p.alias_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    const tFilter1 = performance.now();
    console.log(`[searchProductOffline] Filtering 130k products took: ${(tFilter1 - tFilter0).toFixed(2)} ms`);

    console.log(`[searchProductOffline] Total offline search took: ${(performance.now() - overallStart).toFixed(2)} ms`);
    return result;
  } catch (err) {
    console.error("Error searching product offline:", err);
    console.log(`[searchProductOffline] Total offline search (failed) took: ${(performance.now() - overallStart).toFixed(2)} ms`);
    return [];
  }
}
