...
# ==============================================================================
# GLOBAL CONFIGURATION VARIABLES FOR ACCOUNTS RECEIVABLE
# ==============================================================================
RECEIVABLE_PARENT_CODE = 112
RECEIVABLE_LEVEL = 3
...
def get_accounts_receivable_data(
    ...
    till_date,
    city_filter="",
    parent_code=RECEIVABLE_PARENT_CODE,
    parent_level=RECEIVABLE_LEVEL,
):
    """
    Computes Accounts Receivable data up to `till_date` for all child/detail
    accounts under parent_code (default 112).
    """
    ...
    columns = [
        ...
        {"key": "balance", "label": "Receivable Balance", "width": "16%"},
    ]
    can_view, message = get_user_perms(request, 'see_receivables_statements')
    if not can_view:
    ...
    net_balance = tot_debit - tot_credit
    ...
    summary = {
        ...
        "total_receivable": f"{tot_receivable:,.2f}",
...
def accounts_receivable(request):
    ...
    data, columns, summary, distinct_cities = get_accounts_receivable_data(
        request,
        till_date=till_date,
    ...
    return render(request, "reports/accounts_receivable.html", context)
