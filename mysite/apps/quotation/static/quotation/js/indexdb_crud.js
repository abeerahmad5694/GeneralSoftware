...
const STORE_NAME = "pendingQuotationBills";
...
async function syncoffline_bills() {
  ...
  try {
    ...
    console.error("[Quotation Sync] Failed to read local DB:", dbErr);
    ...
    await Promise.all(
      batchGroup.map(async (chunk) => {
        ...
        try {
          ...
          console.error("[Quotation Sync] Network/HTTP error for batch:", networkErr);
        ...
        if (data && data.success) {
          ...
          try {
            ...
            console.error("[Quotation Sync] Failed to delete local records after successful sync:", deleteErr);
          ...
          console.error("[Quotation Sync] Server rejected batch:", serverMsg, "| Saved:", data?.saved_count, "| Failed:", data?.failed_count);
          totalFailed += chunk.length;
          chunk.forEach(b => failedBillDetails.push({
            local_id: b.local_id,
            reason: serverMsg
  ...
  if (totalFailed === 0 && totalSynced > 0) {
    ...
    console.warn("[Quotation Sync] Failures:", failedBillDetails);
